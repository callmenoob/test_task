...
from components.game_state import GameState, GameStateMethods
...
class GamePlayPage(BasePage):
    ...
    @property
    def game_state(self):
        return self.altdriver.find_object(By.NAME, 'Game', enabled=False)
    ...
    def get_current_life(self):
        return int(self.character.get_component_property('CharacterInputController', 'currentLife', CS_ASSEMBLY))

    def jump(self):
        self.character.call_component_method('CharacterInputController', 'Jump', CS_ASSEMBLY)

    def move_right(self):
        self.character.call_component_method('CharacterInputController', 'ChangeLane', CS_ASSEMBLY, parameters=['1'])

    def move_left(self):
        self.character.call_component_method('CharacterInputController', 'ChangeLane', CS_ASSEMBLY, parameters=['-1'])

    def skip_tutorial(self):
        GameState.call_component_method(self.game_state, GameStateMethods.FINISH_TUTORIAL)

    def avoid_obstacles(self, number_of_obstacles=10, invincible=False):
        character = self.character

        if invincible:
            character.call_component_method('CharacterInputController', 'CheatInvincible', CS_ASSEMBLY,
                                            parameters=['true'])
